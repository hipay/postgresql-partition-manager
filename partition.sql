...
create or replace function partition.partitionne_dyn()
returns trigger
language plpgsql 
as $BODY$
 declare
 ...
 begin 
   ...
   THEN
     ...
     select t.keycolumn, p.to_char_pattern into v_column, v_pattern 
      ...
      where t.schemaname = TG_TABLE_SCHEMA and t.tablename = TG_TABLE_NAME ;  

     EXECUTE 'SELECT to_char( (' || quote_literal(NEW) || '::' || TG_RELID::regclass || ').' || quote_ident( v_column ) || ', ' || quote_literal( v_pattern)  || ') ' INTO part ;
 
     v_insert = 'INSERT INTO ' || TG_TABLE_SCHEMA || '.' || TG_TABLE_NAME || '_' || part 
       || ' SELECT (' || quote_literal(textin(record_out(NEW)))
       || '::' || TG_TABLE_SCHEMA || '.' || TG_TABLE_NAME || ').*;' ;
 --     raise notice 'INSERT : % ' , v_insert ; 
     execute v_insert ; 
...
create or replace function partition.partitionne()
returns trigger
language plpgsql 
as $BODY$
 declare
 ...
 begin 
   ...
   THEN
     ...
     then 
       ...
       raise exception 'Missing Key Column and Date Pattern' ;
     end if;
...
create or replace function partition.partitionne_yearly()
returns trigger
language plpgsql 
as $BODY$
 declare
 ...
 begin 
   ...
   THEN
     ...
     SELECT to_char( new.ev_date, 'YYYY') INTO part ;
...
create or replace function partition.partitionne_monthly()
returns trigger
language plpgsql 
as $BODY$
 declare
 ...
 begin 
   ...
   THEN
     ...
     SELECT to_char( NEW.ev_date, 'YYYYMM') INTO part ;
...
create or replace function partition.partitionne_daily()
returns trigger
language plpgsql 
as $BODY$
 declare
 ...
 begin 
   ...
   THEN
     ...
     SELECT to_char( NEW.ev_date, 'YYYYMMDD') INTO part ;
